  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error('Failed to fetch feedback images')
    ...
    <div>
          ...
          {loading ? (
            ...
            <div className="max-w-md mx-auto text-center py-16 px-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-soft">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {error ? 'Feedback Gallery Temporarily Unavailable' : 'No Feedback Images Available'}
              </h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                {error
                  ? 'We are currently unable to retrieve patient feedback visuals. Please try again later.'
                  : 'There are currently no patient feedback images in our collection. Please check back soon.'}
              </p>
              <button
      ...
      {lightbox !== null && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Feedback image lightbox"
        >
          <button
            onClick={closeLightbox}
